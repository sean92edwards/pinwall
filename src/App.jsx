...
function HorizontalWall({session,muted,editing,setEditing,username}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:"calc(-1 * env(safe-area-inset-bottom, 0px))",display:"flex",flexDirection:"column"}}>
      ...
      {editing&&<div className="wall-toolbar" style={{position:"fixed",bottom:"calc(24px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",zIndex:200,display:"flex",alignItems:"center",gap:8,animation:"slideUp 0.2s ease"}}>
        <div className="tb-pill" style={{display:"flex",alignItems:"center",gap:2,background:"rgba(255,253,248,0.97)",borderRadius:30,padding:6,border:"1px solid #e0d5c0",boxShadow:"0 4px 20px rgba(0,0,0,0.18)",backdropFilter:"blur(8px)"}}>
          <div style={{position:"relative"}}>
            ...
            {showPhotoMenu&&!isMobile&&<div onClick={e=>e.stopPropagation()} style={{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:170}}>
          ...
          <div style={{position:"relative"}}>
            <button onClick={()=>{setShowStickers(s=>!s);setShowPhotoMenu(false);setShowNoteMenu(false);setDoodling(false);setErasing(false);}} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"7px 15px",borderRadius:24,border:"none",background:showStickers?"#ece4d4":"transparent",fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>{if(!showStickers)e.currentTarget.style.background="#ece4d4";}} onMouseLeave={e=>{if(!showStickers)e.currentTarget.style.background="transparent";}}><span style={{fontSize:15}}>⭐</span>Sticker</button>
            {showStickers&&!isMobile&&<div onClick={e=>e.stopPropagation()} style={{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:14,padding:12,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:264}}>
          ...
          <div style={{width:1,height:20,background:"#d8cdb8"}}/>
          <div style={{position:"relative"}}>
            <button onClick={()=>{setShowNoteMenu(s=>!s);setShowPhotoMenu(false);setShowStickers(false);setDoodling(false);setErasing(false);}} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"7px 15px",borderRadius:24,border:"none",background:showNoteMenu?"#ece4d4":"transparent",fontFamily:"'Nunito',sans-serif",fontSize:13,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>{if(!showNoteMenu)e.currentTarget.style.background="#ece4d4";}} onMouseLeave={e=>{if(!showNoteMenu)e.currentTarget.style.background="transparent";}}><span style={{fontSize:15}}>📝</span>Note</button>
            {showNoteMenu&&!isMobile&&<div onClick={e=>e.stopPropagation()} style={{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:160}}>
        ...
        {doodling&&!isMobile&&<div style={{position:"absolute",left:"50%",transform:"translateX(-50%)",bottom:56,display:"flex",gap:6,alignItems:"center",background:"rgba(0,0,0,0.8)",borderRadius:20,padding:"6px 12px",zIndex:200}}>
      ...
      {editing&&isMobile&&showPhotoMenu&&<div onClick={()=>setShowPhotoMenu(false)} style={{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:170}}>
        <label style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>🖼️ Polaroid<input type="file" accept="image/*" multiple style={{display:"none"}} onChange={e=>{photoModeRef.current='polaroid';Array.from(e.target.files).forEach((f,i)=>setTimeout(()=>addPhoto(f),i*100));e.target.value='';setShowPhotoMenu(false);}} disabled={uploading}/></label>
        <label style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>📐 Frameless<input type="file" accept="image/*" multiple style={{display:"none"}} onChange={e=>{photoModeRef.current='frameless';Array.from(e.target.files).forEach((f,i)=>setTimeout(()=>addPhoto(f),i*100));e.target.value='';setShowPhotoMenu(false);}} disabled={uploading}/></label>
      </div>}
      {editing&&isMobile&&showStickers&&<div onClick={e=>e.stopPropagation()} style={{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:14,padding:12,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:"calc(100vw - 32px)",maxWidth:320,maxHeight:"50vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{fontFamily:"'Nunito',sans-serif",fontSize:10,fontWeight:700,color:"#b9a888",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:8}}>Tap to add</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
          {STICKERS.map((em,i)=><button key={i} onClick={()=>addSticker(em)} style={{background:"#f6efe2",border:"1px solid #ece1cf",borderRadius:9,padding:"5px 6px",fontSize:20,cursor:"pointer",lineHeight:1,transition:"transform 0.12s"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.25)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>{em}</button>)}
        </div>
      </div>}
      {editing&&isMobile&&showNoteMenu&&<div onClick={e=>e.stopPropagation()} style={{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:160}}>
        <button onClick={()=>{addBubble();setShowNoteMenu(false);}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer",textAlign:"left"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>📝 Sticky Note</button>
        <button onClick={()=>{addSpeechBubble();setShowNoteMenu(false);}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer",textAlign:"left"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>💬 Speech Bubble</button>
      </div>}
      {editing&&isMobile&&doodling&&<div style={{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",display:"flex",gap:4,alignItems:"center",background:"rgba(0,0,0,0.8)",borderRadius:20,padding:"6px 10px",zIndex:201,maxWidth:"calc(100vw - 16px)",overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        {["#1a1a1a","#e63980","#e63946","#ff6b35","#2a9d8f","#e9c46a","#457b9d","#7b2d8b","#4a9e4a","#ff9ecd","#ffffff"].map(c=><div key={c} onClick={()=>setDoodleColor(c)} style={{width:18,height:18,borderRadius:"50%",background:c,border:doodleColor===c?"2px solid #4a90e2":"2px solid rgba(255,255,255,0.3)",cursor:"pointer",flexShrink:0}}/>)}
        <div style={{width:1,height:18,background:"rgba(255,255,255,0.2)",margin:"0 2px",flexShrink:0}}/>
        {[2,4,8].map(w=><div key={w} onClick={()=>setDoodleWidth(w)} style={{width:22,height:22,borderRadius:"50%",background:doodleWidth===w?"rgba(255,255,255,0.2)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><div style={{width:w*2,height:w*2,borderRadius:"50%",background:"white"}}/></div>)}
        <div style={{width:1,height:18,background:"rgba(255,255,255,0.2)",margin:"0 2px",flexShrink:0}}/>
        <button onClick={()=>{const last=items.find(i=>i.type==='doodle');if(last)setItems(p=>p.filter(i=>i.id!==last.id));}} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"3px 8px",color:"#fff",fontFamily:"'Lato',sans-serif",fontSize:9,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>Undo</button>
        <button onClick={()=>{clearedDoodles.current=items.filter(i=>i.type==='doodle'||i.type==='markertext');setItems(p=>p.filter(i=>i.type!=='doodle'&&i.type!=='markertext'));}} style={{background:"#e63946",border:"none",borderRadius:12,padding:"3px 8px",color:"#fff",fontFamily:"'Lato',sans-serif",fontSize:9,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>Clear</button>
        <button onClick={()=>{if(clearedDoodles.current.length){setItems(p=>[...clearedDoodles.current,...p]);clearedDoodles.current=[];}}} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"3px 8px",color:"#fff",fontFamily:"'Lato',sans-serif",fontSize:9,fontWeight:700,cursor:clearedDoodles.current.length?"pointer":"default",opacity:clearedDoodles.current.length?1:0.4,whiteSpace:"nowrap",flexShrink:0}}>Redo</button>
        <div style={{width:1,height:18,background:"rgba(255,255,255,0.2)",margin:"0 2px",flexShrink:0}}/>
        <button onClick={()=>setErasing(e=>!e)} style={{background:erasing?"#fff":"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"3px 8px",color:erasing?"#1a1a1a":"#fff",fontFamily:"'Lato',sans-serif",fontSize:9,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>🧹</button>
        <button onClick={()=>{const c=centerWorld();const id=Date.now();setItems(p=>[{id,type:'markertext',text:"",cx:c.x,cy:c.y,rot:0,color:doodleColorRef.current,scale:1,zIndex:maxZ+1},...p]);setMaxZ(z=>z+1);setDoodling(false);setEditing(true);setSelected(id);setEditingText(id);}} style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:12,padding:"3px 8px",color:"#fff",fontFamily:"'Permanent Marker',sans-serif",fontSize:9,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>Aa</button>
      </div>}
        ...
        {editing&&<div style={{position:"absolute",left:0,top:0,transformOrigin:"0 0",transform:`translate(${view.x}px,${view.y}px) scale(${view.zoom})`,zIndex:9,pointerEvents:"auto"}}>
          {items.filter(i=>i.type==='audio').map(item=>{
            ...
            return(<div key={item.id} style={{position:"absolute",left:item.cx,top:item.cy,transform:"translate(-50%,-50%)",cursor:"grab",userSelect:"none"}} onMouseDown={e=>startDrag(e,item.id)} onTouchStart={e=>startDrag(e,item.id)} onClick={e=>{e.stopPropagation();setSelected(item.id);}}>
              ...
              {isSel&&<div style={{position:"absolute",top:-44,left:"50%",transform:"translateX(-50%)",display:"flex",gap:5,alignItems:"center",background:"rgba(0,0,0,0.8)",borderRadius:18,padding:"5px 8px",zIndex:20}} onClick={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()} onTouchStart={e=>e.stopPropagation()}>
                ...
                {!(/iPhone|iPad|iPod/.test(navigator.userAgent))&&<><input type="range" min="0" max="100" value={Math.round((item.volume||1)*100)} onChange={e=>setItems(p=>p.map(i=>i.id===item.id?{...i,volume:e.target.value/100}:i))} onTouchStart={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()} style={{width:60,height:20,accentColor:"#e9c46a",cursor:"pointer"}}/>
...
export default function Pinwall(){
  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",background:"#f3ead9",height:"100%"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Caveat:wght@400;500;600;700&family=Nunito:wght@400;600;700;800;900&family=Permanent+Marker&display=swap');*{box-sizing:border-box;margin:0;padding:0;}::-webkit-scrollbar{width:6px;height:6px}::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.15);border-radius:3px}@media(max-width:768px){.pinwall-nav{height:auto!important;padding:calc(6px + env(safe-area-inset-top)) 8px 6px!important}.pinwall-nav .logo-text{font-size:16px!important}.pinwall-nav button{padding:5px 10px!important;font-size:10px!important}.pinwall-nav .nav-circle{width:30px!important;height:30px!important;font-size:12px!important}.zoom-controls{display:none!important}.wall-toolbar{padding:6px 10px!important}.wall-toolbar .tb-pill{padding:3px!important;gap:0!important}.wall-toolbar .tb-btn{padding:5px 8px!important;font-size:0!important;gap:4px!important}.wall-toolbar .tb-btn span{font-size:12px!important}.wall-toolbar .tb-pill>div>button,.wall-toolbar .tb-pill>div>label,.wall-toolbar .tb-pill>button,.wall-toolbar .tb-pill>label{font-size:0!important}.wall-toolbar .tb-pill>div>button span,.wall-toolbar .tb-pill>div>label span,.wall-toolbar .tb-pill>button span,.wall-toolbar .tb-pill>label span{font-size:14px!important}.wall-toolbar{bottom:calc(24px + env(safe-area-inset-bottom))!important}.photo-modal-img{min-height:55vh!important}.photo-modal-comments{width:100%!important;max-height:40vh!important}.photo-modal-close{top:10px!important;right:10px!important}.photo-modal{flex-direction:column!important}.export-btn{display:none!important}}@keyframes slideUp{from{transform:translateX(-50%) translateY(20px);opacity:0}to{transform:translateX(-50%) translateY(0);opacity:1}}`}</style>
      <div className="pinwall-nav" style={{position:"fixed",top:0,left:0,right:0,zIndex:300,padding:"calc(16px + env(safe-area-inset-top)) 20px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",pointerEvents:"none"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,pointerEvents:"auto"}}>
          <button onClick={()=>setEditing(e=>!e)} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",padding:"6px 12px",borderRadius:24,border:"none",cursor:"pointer",fontSize:11,fontFamily:"'Nunito',sans-serif",fontWeight:700,background:editing?"#2a9d8f":"rgba(30,30,30,0.85)",color:"#fff",boxShadow:"0 2px 8px rgba(0,0,0,0.3)",gap:4}}>{editing?"✓ Done":"✏️ Edit wall"}</button>
          <div style={{position:"relative"}}>
            <button className="nav-circle" onClick={()=>{setMuted(m=>!m);setAudioHint(false);}} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:34,height:34,borderRadius:"50%",border:"none",background:"rgba(30,30,30,0.7)",color:"#fff",fontSize:14,cursor:"pointer",boxShadow:"0 2px 6px rgba(0,0,0,0.25)"}}>{muted?"🔇":"🔊"}</button>
          ...
          <div style={{position:"relative"}}>
            <button className="nav-circle" onClick={()=>setShowFriendsPanel(s=>!s)} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:34,height:34,borderRadius:"50%",border:"none",background:showFriendsPanel?"#2a9d8f":"rgba(30,30,30,0.7)",color:"#fff",fontSize:14,cursor:"pointer",boxShadow:"0 2px 6px rgba(0,0,0,0.25)"}}>👥</button>
