...
function HorizontalWall({session,muted,editing,setEditing,username}){
  ...
  const lod=zoom>=(isMobile?0.06:0.12)?'full':'low';
  ...
  useEffect(()=>{
    const lodThreshold=isMobile?0.06:0.12;
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,height:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      {editing&&<div className="wall-toolbar" style={{position:"fixed",bottom:"calc(24px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",zIndex:200,display:"flex",alignItems:"center",gap:8,animation:"slideUp 0.2s ease"}}>
        <div className="tb-pill" style={{display:"flex",alignItems:"center",gap:2,background:"rgba(255,253,248,0.97)",borderRadius:30,padding:6,border:"1px solid #e0d5c0",boxShadow:"0 4px 20px rgba(0,0,0,0.18)",backdropFilter:"blur(8px)"}}>
          <div style={{position:"relative"}}>
            ...
            {showPhotoMenu&&<div onClick={e=>e.stopPropagation()} style={isMobile?{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:170}:{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:170}}>
          ...
          <div style={{position:"relative"}}>
            ...
            {showStickers&&<div onClick={e=>e.stopPropagation()} style={isMobile?{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:14,padding:12,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:"calc(100vw - 32px)",maxWidth:320,maxHeight:"50vh",overflowY:"auto"}:{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:14,padding:12,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:264}}>
          ...
          <div style={{position:"relative"}}>
            ...
            {showNoteMenu&&<div onClick={e=>e.stopPropagation()} style={isMobile?{position:"fixed",bottom:"calc(76px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:160}:{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:200,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:160}}>
        ...
        {doodling&&<div style={{position:"absolute",left:"50%",transform:"translateX(-50%)",bottom:56,display:"flex",flexDirection:(isMobile&&isPortrait)?"column":"row",gap:(isMobile&&isPortrait)?4:6,alignItems:"center",background:"rgba(0,0,0,0.8)",borderRadius:20,padding:(isMobile&&isPortrait)?"8px 10px":"6px 12px",zIndex:200,maxWidth:(isMobile&&isPortrait)?"calc(100vw - 24px)":"none"}}>
          <div style={{display:"flex",gap:(isMobile&&isPortrait)?4:6,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
          ...
          {!(isMobile&&isPortrait)&&<div style={{width:1,height:18,background:"rgba(255,255,255,0.2)",margin:"0 4px"}}/>}
          ...
          {(isMobile&&isPortrait)&&<div style={{width:"80%",height:1,background:"rgba(255,255,255,0.15)",margin:"2px 0"}}/>}
          <div style={{display:"flex",gap:(isMobile&&isPortrait)?4:6,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
          {!(isMobile&&isPortrait)&&<div style={{width:1,height:18,background:"rgba(255,255,255,0.2)",margin:"0 4px"}}/>}
      ...
      <div style={{position:"absolute",inset:0,boxShadow:"inset 0 0 90px rgba(0,0,0,0.22)",pointerEvents:"none",zIndex:5}}/>
        <div style={{position:"absolute",left:0,top:0,transformOrigin:"0 0",transform:`translate(${view.x}px,${view.y}px) scale(${view.zoom})`,pointerEvents:(doodling&&!erasing)?"none":"auto"}}>
          {sorted.map(item=>{
            ...
            if(lod==='low'&&item.type==='sticker') return null;
            if(lod==='low'&&item.type!=='doodle'&&item.type!=='markertext'){
              const w=(item.type==='bubble'?150*(item.scale||1):(item.w||148));
              const h=(item.type==='bubble'?70*(item.scale||1):(item.h||148));
              const col=item.color||"#ffffff";
              return(<div key={item.id} style={{position:"absolute",left:item.cx,top:item.cy,width:w,height:h,transform:`translate(-50%,-50%) rotate(${item.rot||0}deg)`,background:col,borderRadius:6,zIndex:item.zIndex||1,boxShadow:"0 6px 14px rgba(0,0,0,0.18)"}}/>);
            }
      ...
      {viewPhoto&&<div className="photo-modal" onClick={()=>{setViewPhoto(null);setPhotoComments([]);setCommentText("");}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:1000,display:"flex",alignItems:"stretch"}}>
        ...
        <div className="photo-modal-comments" onClick={e=>e.stopPropagation()} style={{width:300,background:"#fff",display:"flex",flexDirection:"column",cursor:"default"}}>
          <div style={{padding:"12px 16px",borderBottom:"1px solid #eee",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontFamily:"'Nunito',sans-serif",fontSize:14,fontWeight:800}}>Comments</span>
            <button onClick={()=>{setViewPhoto(null);setPhotoComments([]);setCommentText("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:22,color:"#aaa",lineHeight:1,padding:"0 0 0 12px"}}>✕</button>
          </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",fontFamily:"'Nunito',sans-serif"}}>
      ...
      {viewPhoto&&<div onClick={()=>setViewPhoto(null)} style={{position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.96)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"zoom-out"}}>
        ...
        <div onClick={()=>setViewPhoto(null)} style={{position:"absolute",top:"calc(env(safe-area-inset-top, 0px) + 20px)",right:"calc(env(safe-area-inset-right, 0px) + 20px)",color:"white",fontSize:32,cursor:"pointer",lineHeight:1,opacity:0.7,padding:8}}>✕</div>
...
export default function Pinwall(){
  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",background:"#c6a06a",position:"fixed",top:0,left:0,right:0,bottom:0,height:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Caveat:wght@400;500;600;700&family=Nunito:wght@400;600;700;800;900&family=Permanent+Marker&display=swap');*{box-sizing:border-box;margin:0;padding:0;}::-webkit-scrollbar{width:6px;height:6px}::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.15);border-radius:3px}@media(max-width:768px){.pinwall-nav{height:auto!important;padding:calc(14px + env(safe-area-inset-top, 20px)) 8px 6px!important}.pinwall-nav .logo-text{font-size:16px!important}.pinwall-nav button{padding:5px 10px!important;font-size:10px!important}.pinwall-nav .nav-circle{width:30px!important;height:30px!important;font-size:12px!important}.zoom-controls{display:none!important}.wall-toolbar{padding:6px 10px!important}.wall-toolbar .tb-pill{padding:3px!important;gap:0!important}.wall-toolbar .tb-btn{padding:5px 8px!important;font-size:0!important;gap:4px!important}.wall-toolbar .tb-btn span{font-size:12px!important}.wall-toolbar .tb-pill>div>button,.wall-toolbar .tb-pill>div>label,.wall-toolbar .tb-pill>button,.wall-toolbar .tb-pill>label{font-size:0!important}.wall-toolbar .tb-pill>div>button span,.wall-toolbar .tb-pill>div>label span,.wall-toolbar .tb-pill>button span,.wall-toolbar .tb-pill>label span{font-size:14px!important}.wall-toolbar{bottom:calc(24px + env(safe-area-inset-bottom))!important}.photo-modal-img{min-height:55vh!important}.photo-modal-comments{width:100%!important;max-height:40vh!important}.photo-modal{flex-direction:column!important}.export-btn{display:none!important}}@keyframes slideUp{from{transform:translateX(-50%) translateY(20px);opacity:0}to{transform:translateX(-50%) translateY(0);opacity:1}}`}</style>
      <div className="pinwall-nav" style={{position:"fixed",top:0,left:0,right:0,zIndex:300,padding:"calc(16px + env(safe-area-inset-top, 20px)) 20px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",pointerEvents:"none"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,pointerEvents:"auto"}}>
          <span style={{fontSize:22,lineHeight:1,filter:"drop-shadow(0 2px 4px rgba(0,0,0,0.3))"}}>📌</span>
