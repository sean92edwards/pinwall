...
function HorizontalWall({session,muted,editing,setEditing,username}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:"calc(-1 * env(safe-area-inset-bottom, 0px))",display:"flex",flexDirection:"column"}}>
      ...
      {editing&&<div className="wall-toolbar" style={{position:"fixed",bottom:"calc(24px + env(safe-area-inset-bottom))",left:0,right:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",gap:8,pointerEvents:"none"}}>
        <div className="tb-pill" style={{display:"flex",alignItems:"center",gap:2,background:"rgba(255,253,248,0.97)",borderRadius:30,padding:6,border:"1px solid #e0d5c0",boxShadow:"0 4px 20px rgba(0,0,0,0.18)",backdropFilter:"blur(8px)",pointerEvents:"auto"}}>
      ...
      {editing&&showPhotoMenu&&<div onClick={()=>setShowPhotoMenu(false)} style={{position:"fixed",bottom:"calc(80px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:170}}>
        <label style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"} onClick={e=>e.stopPropagation()}>🖼️ Polaroid<input type="file" accept="image/*" multiple style={{display:"none"}} onChange={e=>{photoModeRef.current='polaroid';Array.from(e.target.files).forEach((f,i)=>setTimeout(()=>addPhoto(f),i*100));e.target.value='';setShowPhotoMenu(false);}} disabled={uploading}/></label>
        <label style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",border:"none",background:"transparent",borderRadius:8,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:700,color:"#3a3327",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f3ecdf"} onMouseLeave={e=>e.currentTarget.style.background="transparent"} onClick={e=>e.stopPropagation()}>📐 Frameless<input type="file" accept="image/*" multiple style={{display:"none"}} onChange={e=>{photoModeRef.current='frameless';Array.from(e.target.files).forEach((f,i)=>setTimeout(()=>addPhoto(f),i*100));e.target.value='';setShowPhotoMenu(false);}} disabled={uploading}/></label>
      </div>}
      {editing&&showStickers&&<div onClick={()=>setShowStickers(false)} style={{position:"fixed",bottom:"calc(80px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:14,padding:12,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:"min(264px, calc(100vw - 32px))"}}>
      ...
      {editing&&showNoteMenu&&<div onClick={()=>setShowNoteMenu(false)} style={{position:"fixed",bottom:"calc(80px + env(safe-area-inset-bottom, 0px))",left:"50%",transform:"translateX(-50%)",background:"#fffdf8",border:"1px solid #ece1cf",borderRadius:12,padding:10,zIndex:201,boxShadow:"0 12px 36px rgba(80,60,20,0.22)",width:160}}>
